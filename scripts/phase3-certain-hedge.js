...
// Minimal .env loader (works from any cwd, no deps).
try {
...
async function main() {
  for (let w = 0; w < Number(WINDOW_LIMIT); w++) {
    ...
    for (let s = 0; s < 12; s++) {
      ...
      // prefer the freshest window with a live book; the P(Up) gate is
      // re-checked at strike time (T-25s)
      win = live.filter((x) => x.bestYesBid > 0n).sort((a, b) => b.secsLeft - a.secsLeft)[0];
      if (win) {
