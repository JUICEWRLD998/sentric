...
- **Deployed (testnet 50312):** `0x3cc0d4e847d605c2bc526bbf9dadb541576d8ad2` — agentIds + Bitfinex array mode + cycleEnabled=true set on-chain; **NOT armed yet** (deployer needs ~33.2 STT; had 17.65 after deploy → faucet top-up pending). NOTE: this deployment predates the confidence-prompt fix → will be REDEPLOYED with the fixed bytecode once funded.
- **LIVE PIPELINE VALIDATED (2026-08-30, real platform, via `contracts/src/TestRequester.sol` + `scripts/test-agent-cycle.js`):** fetchUintArray(Bitfinex,"",8) → Success 1s, candle `[mts, 78236, 79159, 79415, 77928, vol]` → price $79,159.00, change +1.17%, range +1.87%; inferString → "HOLD" (11s); inferNumber → 0 (11s) with the naive prompt, **75 (42s) with the decision embedded in the prompt** (the model is stateless across requests — the confidence prompt MUST restate the decision; fixed in `_onDecided`).
- **Remaining:** fund deployer (~16 STT, GCP faucet 1/request or TG after cooldown) → `node scripts/deploy-brain-phase2.js` (redeploys with fixed bytecode + config + arm) → watch first live cycle: RequestCreated(json) → RequestFinalized → RequestCreated(llm) → RequestFinalized → AuditEvent.
