...
### Phase 2 — DONE (on-chain AI cycle proven on testnet)
- **SentricBrain v2** rewritten: full async decision state machine (Idle → Fetching → Deciding → Scoring) on the real `IAgentRequester` ABI (`contracts/src/lib/IAgentRequester.sol` — replaces the wrong `IAgentPlatform` stub). Per EpochTick: 1x JSON fetch (Bitfinex candle via `fetchUintArray(url,"",8)` — price + 24h change + high/low vol in one request, or CoinGecko scalar mode) → 1x `inferString` (HEDGE/STAND_DOWN/HOLD, deterministic) → 1x `inferNumber` (0-100 confidence, decision restated in prompt — the model is stateless across requests) → `AuditEvent(inputsHash, decision, confidence, asset)`. `handleResponse` gated to the platform + requester check; Failed/TimedOut reset to Idle; one cycle in flight guard; owner-configurable agentIds/fetch-params/fetch-mode/fees; `sweep()` recovers the reserve when disarmed. **29/29 tests pass.**
- **Verified agent facts (docs/agent-ids.md + docs/price-endpoints.md):** json-fetch agentId **13174292974160097713**, llm-inference **12847293847561029384** (same testnet+mainnet; beware 9911223344556677889 = custom aggregator, NOT json-fetch). `getRequestDeposit()` = 0.03 STT floor; real calls fund floor + price×subSize (0.12 json / 0.24 llm, over-deposit rebated). Bitfinex 1D candle array endpoint is the recommended single-request feed (CoinGecko fallback).
- **Deployed (testnet 50312):** SentricBrain v2 `0x9b0ee5aff990d09a099672a621b7ce18d7ac98ec` (deploy tx `0xde1721d1…` @ 475433791; arm tx `0xefe1b49e…` @ 475433857, subscription **14902015**). Config: Bitfinex array mode, agentIds set, 33 STT reserve.
- **MILESTONE MET — one tick → end-to-end AuditEvent, verified on-chain:** EpochTick @ 475433999 → RequestCreated json-fetch (req 12759129) → LLM action (12759131) → LLM confidence (12759132) → **AuditEvent @ 475434019** (tx `0x4678c913…`): `inputsHash 0xd9f9a943…`, **decision "HOLD", confidence 75**, asset BTC(address(1)). Brain returned to Idle; balance 32.482 STT (0.60 fees − ~0.08 rebate, receive() works). The cycle repeats every ~5 min — `scripts/watch-cycle.py` streams the AuditEvents.
- **Live pipeline validation (before arming):** `contracts/src/TestRequester.sol` + `scripts/test-agent-cycle.js` proved fetch (Success 1s, $79,159, +1.17%, +1.87%), action ("HOLD", 11s) and confidence (0→75 after the decision-restatement fix) against the REAL platform.
- **Notes:** Phase-1 brain `0x213714…` disarmed; its ~33 STT is locked (no sweep in v1). Deploy via `scripts/deploy-brain-phase2.js` (viem; forge script `-32602` on this RPC). Fresh faucet key in `.env` (`FAUCET_ADDRESS`/`FAUCET_PRIVATE_KEY`, gitignored) for cooldown-free claims.
- **Next (Phase 3):** venue `placeOrder` + `redeem` + hedge sizing (`size = f(exposure, P(Down))`, max premium per window) — exact `placeOrder` signature still to be read from `binaryModuleWriteAbi`.

### Phase 3 — DONE (on-chain trading + redemption proven on testnet)
- **`SentricVault.sol`** (replaces the stub): real DreamDEX venue wiring via `IBinaryVenue.sol` (ABIs verified against markets-sdk v0.28.1 `tradeAbi.js` — generic `placeOrder` reverts `UseBinaryPlacement`; orders go to the per-market **BinaryPool**). `placeHedge(size, yesPrice)` = `placeBinaryOrder(kind=2 BUY_NO, price=yesPrice ALWAYS YES-side, qty raw 1e6/token, expire=pool.marketExpiryNs(), orderType=2 MARKET/IOC, selfMatching=0, builder=0)` with tick-snap-up / lot-snap-down / min-qty floor; per-epoch premium guard (`block.number/3000` window, `maxPremiumPerWindow`); `redeemSettled(outcomeIdx, amount)` = `finalizeAndRedeem(pool, (pool<<72)|(nonce<<8)|idx, amount, vault)` with the **nonce captured at order time** (pools recycle). `sizeHedge(exposure, P(Down), maxPremium, moveBps)` — `N = exposure×move/(1−P(Down))` capped by `maxPremium/P(Down)`, degenerate inputs → 0. `withdraw()` intentionally reverts (no transfer-anywhere path).
- **`SentricBrain.sol`** Phase-3 additions: `_executeHedge` on LLM `HEDGE` (AuditEvent first, then the order); `manualHedge()` (demo pulse); `manualRedeem(outcomeIdx, amount)`; `manualSetVenue(pool, settlement, collateral, marketId)` (follow rolling windows + grants the new pool escrow approval — **per-pool allowance, missing it = silent revert**); `manualHedgeNow(pool, marketId, downPriceBps, yesPrice)` (ONE tx: re-point + approve + size + place — collapses the timing risk). **55/55 tests pass** (`SentricSizing.t.sol` + `SentricVaultTrade.t.sol` cover sizing math, tick/lot snapping, premium cap, window reset, outcome-id construction, the full tick→fetch→LLM HEDGE→order cycle, and redemption).
- **Deployed (testnet 50312, final stack):** SentricVault `0x5985d3a5321704f352c0753562a09b05762eb4a6` + SentricBrain v3 `0x282dd667d34806af1ac614737bbef03127757218` (vault owned by brain; 10,000 tUSDC funded; pool approval + ERC-6909 settlement operator granted). Deploy via `scripts/deploy-phase3.js` (SKIP_ARM=1 — arm needs the 33 STT reserve; manual path doesn't).
- **MILESTONE MET — vault placed a REAL order and REDEEMED a settled position (both on the explorer):** window **#60567** (op-2 5-min BTC product series, 09:40–09:45 UTC) → `manualHedgeNow` tx `0x7885d2c8…` (gas 1,732,524): BUY_NO 12 tokens @ NO 0.787, order id 147573952589676459746, pool `0x5dF3d8fc…`, nonce 38, premium 9.444 tUSDC → settlement `payoutNumerators=[0, 10_000_000]` (**NO won**, BTC closed below opening) → `manualRedeem(1, 12e6)` tx `0x89def7d6…` (gas 205,597): redeemed 12e6 NO tokens → **vault balance 10,002.57 tUSDC** (10,000 − 9.44 premium + ~12.01 payout = +2.57 USDC real profit). Earlier in the session: hedge on window #60374 (08:30–08:35) filled and expired as insurance (YES won — premium 9.74 USDC was the insurance cost, correct semantics).
- **Verified venue behavior (docs/network-facts.md §2 + docs/venue-live-recipe.md):** `markets(bytes32)` expiry/tradingStart are **UNIX SECONDS** (pool `marketExpiryNs()` is ns); `getBookLevels(bool,uint64)` is a **tuple array** in viem; pools recycle across windows (same address, nonce++); operator cadence is sporadic — op-2 5-min is the product series (~every 5 min), op-4 runs 1-min bursts with gaps; **near-certain windows (P(Up) < ~0.1) have dead books and the pool reverts `0xd48c4403` (no counterparty)** — hedge the liquid zone (P(Up) 0.25–0.75, top-level depth ≥ 25e6 raw); books drain in the final ~30s of a window. Discovery: `scripts/wait-live-window.js`, `scripts/discover-live-btc.js`; live milestone driver: `scripts/phase3-smart-hedge.js`.
- **Next (Phase 4):** chain the phases — tick → fetch → LLM → HEDGE → vault order → auto-redeem (the brain already emits HedgeExecuted after the order); arm the v3 brain (needs ~1 more STT or sweep of the v2 brain); stop-loss/max-premium rails already in the vault.

### Phase 4 — DONE (full-loop contracts + safety rails; autonomous arm pending 1 STT)
- **SentricVault v2** adds the non-custodial rails: `deposit`/`withdraw` (principal tracking; withdraw can't break the active window's funding — keeps ≥ `windowPremiumSpent`); re-entrancy guards (`nonReentrant` on all money paths); `pause`/`unpause` circuit breaker (blocks NEW hedges only — redeem always stays open); `setMaxDailyPremium` (per-`DAY_BLOCKS`=864,000 daily budget, separate from the per-epoch cap); `redeemSettled(pool, outcomeIdx, amount, nonce)` now takes the **explicit pool + order-time nonce** so old positions stay redeemable even after the vault re-points; `lastOrderQtyRaw` records the placed qty for exact redemption.
- **SentricBrain v4** chains the FULL loop: on each EpochTick it first auto-redeems any open position (`_tryRedeemPosition` — reads the market via `pool.market()`, redeems on a NO win and resets the loss streak, logs `HedgeExpired` + advances the streak on a YES win), then runs fetch → LLM → HEDGE → `_executeHedge` → order (`PositionOpened` tracks pool/nonce/qty). Safety: **loss-streak stop-loss** (3 consecutive losing windows halts new hedges with `StopLossEngaged`, owner `resetLossStreak()`), **one-position-at-a-time**, and **try/catch around both order placement and redemption** (`HedgeFailed`/`HedgeRedeemFailed` — a rejected order or unsettled window never wedges the cycle state machine; the next tick retries). Price feed switched to the **Bitfinex 5m candle** so the LLM reacts to the current move ("Recent change/Range" prompt). **71/71 tests pass** (incl. deposit/withdraw/premium-lock, pause, daily budget, auto-redeem win/loss, stop-loss halt, and failure-recovery paths).
- **Deployed (testnet 50312, v4 stack, unarmed):** SentricVault v2 `0xd4fa5efd13d7cb247c26d267014164031c93885f` + SentricBrain v4 `0xb7ce698f31d8ad10a1714f3da701cdc32c58067e` (vault owned by brain; 10,000 tUSDC; maxPremium 10/window; maxDaily 500/day; 5m feed; deploy via `scripts/deploy-phase4.js`).
- **Live validation on the v4 stack:** deposit→withdraw round-trip on the real tUSDC (deposit 100, withdraw 40+60, balance restored) ✓; full hedge→payout cycle via `manualHedgeNow`+`manualRedeem`: window **#60705** (op-2 5-min, nonce 42) BUY_NO 18 tokens @ NO ~0.543, order tx `0x4850c01c…`, **NO won**, redeem tx `0xb51a1e4a…` → **vault 10,008.24 tUSDC** (10,000 − 9.774 premium + ~18 payout = **+8.23 USDC**). Daily-budget tracking verified on-chain (`dailyPremiumSpent` = 9,774,000).
- **Autonomous arm — PENDING ~1 STT funding:** deployer holds 31.17 STT; the reactivity precompile requires ≥ 32 STT on the brain at `arm()` → one faucet claim (GCP 1 STT/request or Discord) then `arm` (33 STT) starts the self-waking loop. NOTE: the v2 brain's ~32.5 STT reserve was consumed by ~24h of live cycles (~0.11 STT/epoch in agent fees + callback gas) — budget the reserve accordingly (a 33 STT reserve ≈ 2–3 days of continuous autonomy; the v1 brain's 33 STT is verifiably locked, no sweep in v1). The equity tracker `scripts/track-equity.js` is ready to capture the drawdown-reduction measurement for the milestone.
- **LP mode:** documented as a stretch design with VERIFIED signatures — `docs/lp-mode-design.md` (`mintSet`/`burnSet` on the pool 0x54657dd2/0x55664dbd; `mintCompleteSet`/`mergeCompleteSet` on the module 0x47dfb781/0xb6354afe). Verdict: skip for the demo — hedge-side is the pitch; LP quoting adds adverse-selection risk with no demo payoff.

### Phase 5 — DONE (frontend: Noviq UI + Framer Motion, all routes build)
- **Component kit** (`apps/web/components/ui/`, 14 components × TSX + CSS module): Button (4 variants/3 sizes, springTap), Card (glassCard+edgeLight, tone ring), Badge (5 tones + dot), Stat (tabular-nums), CodeBlock (copy), Skeleton (framer pulse, reduced-motion aware), Field/Input (mono), Slider (Radix), Switch (framer knob), Toast (Radix), Container/Stack/Grid/PageHeader. All CSS-module + tier-2/3 tokens only, OKLCH, no Tailwind, no keyframes — Noviq-compliant.
- **Data layer** (`apps/web/lib` + `hooks`): wagmi config + injected connector (wallet connect), typed ABIs from the deployed contracts (`lib/abi/brain.json`/`vault.json`), windowed `eth_getLogs` event scanner (800-block windows, ≤150 windows ≈ 3h+ history — VERIFIED live: 5 real AuditEvents recovered from the phase-2 brain across ~37k blocks), live book polling (`getBookLevels` tuple ABI), react-query hooks: `useBrainState` (state/position/loss-streak, 10s), `useVaultState` (balance/premiums/paused, 10s), `useLiveBook` (15s), `useAuditHistory` + `useHedgeTimeline` (30s).
- **Pages:** `/` (hero + CTAs), `/dashboard` (agent status badge + arm/disarm Switch + position card, vault stats, live Up/Down odds bar from the book, owner-only threshold sliders → `setHedgeConfig`, hedge-history table from brain events), `/reason-explorer` (THE hero: AuditEvent receipts — decision badge, confidence Stat, inputsHash, asset, block, expandable raw CodeBlock; brain-address switcher incl. the phase-2 brain's historical receipts), `/styleguide` (tokens + patterns + new Component-kit section with live demos). Nav + ConnectButton (wagmi, STT balance) in the root layout; Framer Motion only (`fadeUp`/`staggerParent` presets, MotionConfig reducedMotion).
